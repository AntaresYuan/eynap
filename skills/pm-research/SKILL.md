---
name: pm-research
description: 产出一份用户调研或竞品分析报告。当用户需要了解目标用户的真实行为与需求、或需要盘点市场上已有的同类产品时使用。产出物是调研报告，含研究问题、方法、发现与结论，落在 docs/04_RESOURCES/。触发词：做份调研、竞品分析、市场上有什么类似的、用户到底怎么用的、这个方向有人做过吗、对标产品。不负责：定义产品价值与范围（用 pm-value）、梳理流程（用 pm-flow）、写需求文档（用 pm-prd）。
---
